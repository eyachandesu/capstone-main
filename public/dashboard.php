          ...
          <div x-show="open" class="absolute right-0 mt-3 w-72 bg-white rounded-xl shadow-lg ring-1 ring-gray-100 overflow-hidden z-50 text-gray-700" style="display: none;">
             ...
             <ul class="max-h-64 overflow-y-auto">
               <?php if ($newOrdersNotif > 0): ?>
                 <li class="flex items-center gap-2 px-4 py-3 hover:bg-gray-50 border-b"><span class="text-[var(--rose)]">🛒</span> <span><?= $newOrdersNotif ?> new order(s)</span></li>
               <?php endif; ?>
               <?php if ($lowStockNotif > 0): ?>
                 <li class="flex items-center gap-2 px-4 py-3 hover:bg-gray-50 border-b"><span class="text-yellow-500">📦</span> <span><?= $lowStockNotif ?> low stock item(s)</span></li>
               <?php endif; ?>
